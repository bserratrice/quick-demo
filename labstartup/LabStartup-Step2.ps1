...
Get-Cluster "VMware-Cloud" | Get-AlarmDefinition -Name "vSAN health alarm 'vSAN disk balance'" | Set-AlarmDefinition -enabled:$false | Out-Null
...
Write-Output "Disable Host memory usage alarm and reset Network Alarms"
Get-VMHost | ForEach-Object {
    $_ | Get-AlarmDefinition -Name "Host memory usage" | Set-AlarmDefinition -enabled:$false | Out-Null
    $_ | Get-AlarmDefinition -Name "Network connectivity lost" | Set-AlarmDefinition -enabled:$false | Out-Null
    $_ | Get-AlarmDefinition -Name "Network uplink redundancy lost" | Set-AlarmDefinition -enabled:$false | Out-Null
    Start-Sleep -Seconds 1
    $_ | Get-AlarmDefinition -Name "Network connectivity lost" | Set-AlarmDefinition -enabled:$true | Out-Null
    $_ | Get-AlarmDefinition -Name "Network uplink redundancy lost" | Set-AlarmDefinition -enabled:$true | Out-Null
}